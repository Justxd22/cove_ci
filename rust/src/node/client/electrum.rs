...
use bdk_electrum::{
    BdkElectrumClient,
    electrum_client::{
        self, Client, Config, ConfigBuilder, ElectrumApi as _, Param, Socks5Config,
    },
};
...
impl ElectrumClient {
    ...
    pub async fn new_from_node_and_options(
        ...
        let inner_client = cove_tokio::unblock::run_blocking(move || Client::from_config(&url, config))
            .await
    ...
    fn default_options() -> NodeClientOptions {
        NodeClientOptions {
            ...
            ..NodeClientOptions::default()
        }
    }

    fn connection_config(options: &NodeClientOptions) -> Config {
        let socks5_addr = if options.use_tor {
            Some(format!("{}:{}", options.tor_external_host, options.tor_external_port))
        } else {
            None
        };

        ConfigBuilder::new()
            .socks5(socks5_addr.map(Socks5Config::new))
            .build()
    }
}
